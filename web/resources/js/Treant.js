		...
		*/
		...
		positionNodes: function() {
			...
			var self = this,
				...
				treeHeight = treeSize.y.max - treeSize.y.min +1000,
			...
			for(i =0, len = this.nodeDB.db.length; i < len; i++) {
				...
				if(collapsedParent) {
					...
					node.nodeDOM.style.top = node.Y + 100+'px';
	...
	TreeNode.prototype = {
		...
		connectorPoint: function(startPoint) {
			...
			if (orient == 'NORTH') {
				...
				point.y = (startPoint) ? this.Y + this.height +100 : this.Y +100;

			} else if (orient == 'SOUTH') {

				point.x = (this.pseudo) ? this.X - this.Tree().CONFIG.subTeeSeparation/2 : this.X + this.width/2;
				point.y = (startPoint) ? this.Y  +100: this.Y + this.height;

			} else if (orient == 'EAST') {

				point.x = (startPoint) ? this.X : this.X + this.width;
				point.y = (this.pseudo) ? this.Y  +100 - this.Tree().CONFIG.subTeeSeparation/2 : this.Y + this.height/2;

			} else if (orient == 'WEST') {

				point.x = (startPoint) ? this.X + this.width : this.X;
				point.y =  (this.pseudo) ? this.Y +100 - this.Tree().CONFIG.subTeeSeparation/2 : this.Y +100+ this.height/2;
			}
			return point;
		},
		...
		pathStringThrough: function() { // get the geometry of a path going through the node
			...
			return ["M", startPoint.x+","+startPoint.y, 'L', endPoint.x+","+(endPoint.y+50)].join(" ");
		},
		...
		if (!this.pseudo) {
			if (!this.nodeInnerHTML) {
				...
				if(this.image) {
					...
					//div.className = 'node-html-img';
					//div.innerHTML= '<img data-personId="http://10.0.0.34:7001/BiLogin/imageprsn/KisiResimWithPersIdServlet?personId=3009">';
