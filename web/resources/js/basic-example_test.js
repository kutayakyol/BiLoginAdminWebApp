...
$(document).ready(function(){
	$("#namebutton").click(function(event){
		...
		$.when($.ajax({
			type:"GET", 
			dataType: 'json',
			url: "http://10.0.0.34:7001/BiLogin/rest/getOrgName/"+$("#p_org_tip option:selected" ).val(), 
...
$(document).ready(function(){
	$.when($.ajax({
		type:"GET", 
		dataType: 'json',
		url: "http://10.0.0.34:7001/BiLogin/rest/getOrgType", 
		success: function(data) {
			   //localStorage.setItem('data4', JSON.stringify(data))
			   orgtype=data;
		return data;
...
$(document).ready(function(){
	$("#buttonsub").click(function(event){
		...
		$.when($.ajax({
			type:"GET", 
			dataType: 'json',
			url: "http://10.0.0.34:7001/BiLogin/rest/getOrgChart/"+$("#p_org_tip option:selected" ).val()+"/"+$("#p_org_name option:selected" ).val(), 
			success: function(data) {
				   //localStorage.setItem('data4', JSON.stringify(data))
